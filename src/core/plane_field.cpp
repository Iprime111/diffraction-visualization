...
PlainFieldStorage::PlainFieldStorage(const PlainFieldStorage& other)
    : PlainFieldStorage(other.dataFinish_ - other.data_) {
...
PlainFieldStorage::PlainFieldStorage(PlainFieldStorage&& other) noexcept
    : data_(nullptr), dataFinish_(nullptr) {
    std::swap(dataFinish_, other.dataFinish_);
    std::swap(data_, other.data_);
}

PlainFieldStorage& PlainFieldStorage::operator=(PlainFieldStorage&& other) noexcept {
    std::swap(dataFinish_, other.dataFinish_);
    std::swap(data_, other.data_);
...
PlaneField::PlaneField(std::size_t xSize, std::size_t ySize, const FieldValue* rawData)
    : detail::PlainFieldStorage(xSize * ySize), xSize_(xSize), ySize_(ySize) {
    if (rawData == nullptr) {
